param (
    # Optional: allow passing in additional source paths, default to ./dashboards
    $src = @("./dashboards")
)
...
foreach ($srcPath in $src) {
    ...
    # Recursively find all semantic model files (.pbidataset, .pbism)
    ...
    foreach ($itemFile in $itemsFolders) {
        ...
        if (!(Test-Path $itemPath)) {
            ...
            if (!(Test-Path $itemPath)) {
                throw "Cannot find semantic model definition for '$($itemFile.FullName)'."
            }
        }
